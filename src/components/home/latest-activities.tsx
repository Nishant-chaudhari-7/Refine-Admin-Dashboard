...
const LatestActivities = () => { // for fetching the data 
  ...
  const { data: deals, isLoading: isLoadingDeals } = useList({
    resource: 'deals',
    queryOptions: { enabled: !!dealIds?.length }, // for disabled the query when there no deal ids
    pagination: {
      mode: 'off'
    },
  ...
  return (
    ...
    >
      {isLoading ? ( // loading is on going
        ...
        <List  // when loading is over 
          itemLayout='horizontal'
          dataSource={audit?.data}
          renderItem={(item) => {
            const deal = deals?.data.find(
              (deal) => deal.id === String(item.targetId)
              ) || undefined; // deal id is string and item id is number thats why we use string 
              // or we can use == 
